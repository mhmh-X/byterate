...
凭据只在本机读取，只发往 Anthropic/OpenAI 第一方主机：`api.anthropic.com` / `claude.ai` 和 `chatgpt.com` / `auth.openai.com`。这些额度接口不是公开稳定 API，未来可能变化；如果返回结构无法识别，ByteRate 会报错而不是猜测额度。唯一的额外网络请求是你手动点「检查更新」时查询 GitHub Releases API。除此之外没有任何其他服务器、没有统计埋点、不收集任何数据。token 过期时自动刷新并写回 CLI 的存放位置，两边保持同步登录。
